...
{{ Form::open(array('url' => '/user/verify/'.$user[0]->id, 'class' => 'form-signin', 'name'=>'regForm')) }}
		<h1 class="form-signin-heading" style="text-align:center;">Dane osobowe użytkownika: </h1>
        ...
        {{ Form::select('verified', array(false => 'Niezweryfikowany', true => 'Zweryfikowany'), $user[0]->usr_verified, array('class' => 'form-control'))}}

		{{ Form::submit('Zweryfikuj', array('class' => 'btn btn-lg btn-primary btn-block','ng-disabled'=>'!regForm.$valid', "ng-model"=>"regFormSubmit",)) }}
 {{ Form::close() }}
@stop
